...
class ProjectsController extends ActionController
{
    ...
    {
        $projects = $this->em->getRepository('Buggy\Model\Project')->getProjectList(); //->findAll();
        return array(
            'projects' => $projects, 
            'title' => 'Projects', 
    ...
    public function addAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($this->getRequest()->post()->toArray())) {
                ...
                $this->em->flush();
                $this->getFlashMessenger('projects')->addMessage(
                        array(
                            'success', 
                            'Project: <em>' . $projectRecord->getTitle() . '</em> added succesfully!'
                        ));
                return $this->redirect('projects', 'index');
            } else {
                $messages = array(
                    'error', 
                    'Please controll your input!'
                );
                ...
                if ($this->getRequest()->isPost()) {
                    ...
                    if ($form->isValid($this->getRequest()->post()->toArray())) {
                        ...
                        $this->getFlashMessenger('projects')->addMessage(
                                array(
                                    'success', 
                                    'Project: <em>' . $projectRecord->getTitle() . '</em> saved succesfully!'
                                ));
                        return $this->redirect('projects', 'index');
                    } else {
                        $messages = array(
